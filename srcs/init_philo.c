...
int	set_philos(t_vars *vars)
{
	...
	while (++i < vars->nb_philos)
	{
		vars->philos[i].l_t_e = 0;
		vars->philos[i].philo = i + 1;
		vars->philos[i].n_t_e = 0;
		vars->philos[i].stat = THINK;
		vars->philos[i].vars = vars;
		vars->philos[i].estat = NOTFULL;
...
int	set_variables(int ac, char **av, t_vars *vars)
{
	...
	if (ac == 6)
	{
		vars->nb_tte = ft_atoi(av[5]);
		if (vars->nb_tte <= 0)
			return (1);
	}
	if (vars->nb_philos < 1 || vars->ttd < 0 || vars->tte < 0 || vars->tts < 0)
		return (1);
	return (0);
}
